...
> 文档版本：v1.1\
> 文档日期：2026-07-20\
> 需求状态：M1 身份与用户资料核心链路已完成，MVP 持续开发\
...
- [ ] `AUTH-014 [MVP]` 嵌入式钱包和外部钱包统一创建 Fanora 会话；验收：业务模块只读取统一用户 ID 与主钱包，不判断登录提供商。（统一后端会话结构已完成；外部钱包完整端到端验收待完成）
...
- [ ] `CREATOR-002 [MVP]` 创作者可创建社区；验收：必须填写名称、简介、Logo 和社区标识。（后端接口、权限校验和数据模型已完成，创作者前端待完成）
- [ ] `CREATOR-003 [MVP]` 创作者可编辑自己的社区资料；验收：更新后前端和接口数据一致。（后端所有者校验和更新接口已完成，创作者前端待完成）
- [ ] `CREATOR-004 [MVP]` 社区具备独立积分、任务、等级和 Badge 配置；验收：不同社区的数据不能混算。
- [ ] `CREATOR-005 [P1]` 支持一个社区配置多个管理员；验收：每个管理员具有可撤销的细粒度权限。
- [ ] `COMMUNITY-001 [MVP]` 用户可浏览公开社区列表；验收：支持基本分页和名称搜索。（后端搜索、offset/limit 和 Profile 列表展示已完成，前端搜索与分页控件待完成）
- [x] `COMMUNITY-002 [MVP]` 用户可加入或关注社区；验收：前后端已联调，数据库唯一约束保证重复操作不会产生重复成员记录。
...
- [x] `API-006 [MVP]` `GET /api/v1/communities`：查询公开社区列表，支持名称搜索和 offset/limit。
- [x] `API-007 [MVP]` `POST /api/v1/communities`：创作者或管理员创建社区。
...
- [ ] `API-014 [MVP]` `GET /api/v1/communities/{community_id}/fan-tokens/me`：查询个人 Fan Token 余额和流水。
...
- [x] `API-COMMUNITY-001 [MVP]` `PATCH /api/v1/communities/{community_id}`：仅社区所有者可更新社区资料。
- [x] `API-COMMUNITY-002 [MVP]` `POST /api/v1/communities/{community_id}/join`：当前用户幂等加入公开社区。
...
- [x] `DB-004 [MVP]` 建立 `communities` 表，保存创作者社区资料、所有者和公开状态。
- [x] `DB-005 [MVP]` 建立 `community_members` 表，保存用户加入社区的时间和角色，并防止重复成员记录。

...
2. 首页“加入流程”仍显示旧的“邮箱或社交账号创建钱包”文案，与当前 RainbowKit 直连钱包实现不一致；代码文案需要后续清理。
3. 首页若干图片存在 Next.js 宽高比例 warning，NFT 市场首屏图片存在 LCP priority warning。
4. 当前链上写入主要在 HTTP 请求中等待确认；后台重试、链重组处理、常驻事件监听和完整对账尚未完成。
5. 任务限定 Badge 与演唱会纪念卡的资格领取前端闭环尚未完成。
6. 粉丝 NFT 原始图片仍可作为 Base64 保存在 PostgreSQL；生产环境应迁移独立对象存储并增加配额、扫描和清理。
7. Testnet 运营角色仍需拆分最小权限账户，并将管理员权限迁移至多签。

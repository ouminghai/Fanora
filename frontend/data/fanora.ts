export const recentActivities = [
  {
    id: "fear-and-dreams",
    imageSrc: "/img/fanora/activity-concert.jpg",
    avatarSrc: "/img/fanora/activity-concert.jpg",
    title: "FEAR and DREAMS 巡回演唱会",
    meta: "近期活动 · 演唱会打卡",
    reward: "+500 身份积分",
  },
  {
    id: "new-song-listening",
    imageSrc: "/img/fanora/activity-music.jpg",
    avatarSrc: "/img/fanora/activity-music.jpg",
    title: "新歌共听计划",
    meta: "线上活动 · 每周更新",
    reward: "+120 身份积分",
  },
  {
    id: "city-checkin",
    imageSrc: "/img/fanora/activity-checkin.jpg",
    avatarSrc: "/img/fanora/activity-checkin.jpg",
    title: "城市粉丝见面打卡",
    meta: "线下活动 · 名额有限",
    reward: "+300 身份积分",
  },
  {
    id: "fan-story",
    imageSrc: "/img/fanora/activity-community.jpg",
    avatarSrc: "/img/fanora/activity-community.jpg",
    title: "我的 Eason 故事征集",
    meta: "社区共创 · 投稿进行中",
    reward: "+180 身份积分",
  },
  {
    id: "anniversary-badge",
    imageSrc: "/img/fanora/activity-badge.jpg",
    avatarSrc: "/img/fanora/activity-badge.jpg",
    title: "周年限定 Badge 解锁",
    meta: "链上纪念 · 即将开放",
    reward: "限量 10,000 枚",
  },
];

export const fanMissions = [
  {
    id: 1,
    avatar: "/img/fanora/activity-concert.jpg",
    name: "每日 Fans Club 签到",
    detail: "+20 积分 · 2,846 人参与",
    status: "进行中",
  },
  {
    id: 2,
    avatar: "/img/fanora/activity-checkin.jpg",
    name: "FEAR and DREAMS 打卡",
    detail: "+500 积分 · 演唱会任务",
    status: "进行中",
  },
  {
    id: 3,
    avatar: "/img/fanora/activity-music.jpg",
    name: "新歌连续收听 7 天",
    detail: "+120 积分 · 连续任务",
    status: "进行中",
  },
  {
    id: 4,
    avatar: "/img/fanora/activity-community.jpg",
    name: "分享你的入坑歌曲",
    detail: "+80 积分 · 社区互动",
    status: "进行中",
  },
  {
    id: 5,
    avatar: "/img/fanora/activity-badge.jpg",
    name: "粉丝故事图文征集",
    detail: "+180 积分 · 优质内容加成",
    status: "进行中",
  },
  {
    id: 6,
    avatar: "/img/fanora/activity-concert.jpg",
    name: "周年祝福共创",
    detail: "+150 积分 · 7 月 20 日开启",
    status: "即将开始",
  },
  {
    id: 7,
    avatar: "/img/fanora/activity-music.jpg",
    name: "经典歌词接龙",
    detail: "+60 积分 · 社区挑战",
    status: "进行中",
  },
  {
    id: 8,
    avatar: "/img/fanora/activity-checkin.jpg",
    name: "核心粉丝身份认证",
    detail: "+300 积分 · Agent 审核",
    status: "进行中",
  },
];

export const badgeLevels = [
  {
    id: "bronze",
    imageSrc: "/img/fanora/badge-bronze.svg",
    title: "Bronze Fan Badge",
    level: "Lv.1 新晋粉丝",
    points: "0 - 999 积分",
    benefit: "加入社区 · 基础任务",
    progress: "已解锁",
  },
  {
    id: "silver",
    imageSrc: "/img/fanora/badge-silver.svg",
    title: "Silver Fan Badge",
    level: "Lv.2 活跃粉丝",
    points: "1,000 - 2,999 积分",
    benefit: "活动优先报名 · 专属内容",
    progress: "还差 360 积分",
  },
  {
    id: "gold",
    imageSrc: "/img/fanora/badge-gold.svg",
    title: "Gold Fan Badge",
    level: "Lv.3 核心粉丝",
    points: "3,000 - 7,999 积分",
    benefit: "限定 Badge · 白名单权益",
    progress: "待解锁",
  },
  {
    id: "core",
    imageSrc: "/img/fanora/badge-core.svg",
    title: "Core Fan Badge",
    level: "Lv.4 超级粉丝",
    points: "8,000+ 积分",
    benefit: "核心活动 · 社区共创资格",
    progress: "待解锁",
  },
];

export const fanJourney = [
  {
    id: 1,
    iconSrc: "/img/process/process5.svg",
    title: "1. 加入 Fans Club",
    bgClass: "bg-accent",
    ringColor: "#CDBCFF",
    description: "使用邮箱或社交账号快速加入，系统会在后台为你创建并绑定 Web3 钱包。",
  },
  {
    id: 2,
    iconSrc: "/img/process/process6.svg",
    title: "2. 参与粉丝活动",
    bgClass: "bg-green",
    ringColor: "#C4F2E3",
    description: "完成签到、内容互动、演唱会打卡与社区共创等真实粉丝任务。",
  },
  {
    id: 3,
    iconSrc: "/img/process/process7.svg",
    title: "3. 积累身份积分",
    bgClass: "bg-blue",
    ringColor: "#CDDFFB",
    description: "LangGraph Agent 综合分析活跃度、忠诚度与贡献，持续更新身份积分。",
  },
  {
    id: 4,
    iconSrc: "/img/process/process8.svg",
    title: "4. 解锁链上 Badge",
    bgClass: "bg-red",
    ringColor: "#FFD0D0",
    description: "积分达到门槛后获得铸造资格，把长期支持沉淀为可验证的粉丝身份。",
  },
];

export const fandomProfiles = [
  {
    id: "loyal-fan",
    name: "长期陪伴者",
    images: [
      "/img/fanora/activity-concert.jpg",
      "/img/fanora/activity-music.jpg",
      "/img/fanora/activity-community.jpg",
      "/img/fanora/activity-badge.jpg",
    ],
    avatar: "/img/fanora/activity-concert.jpg",
    identity: "忠诚型粉丝",
    score: "92",
  },
  {
    id: "active-fan",
    name: "现场能量担当",
    images: [
      "/img/fanora/activity-checkin.jpg",
      "/img/fanora/activity-concert.jpg",
      "/img/fanora/activity-badge.jpg",
      "/img/fanora/activity-music.jpg",
    ],
    avatar: "/img/fanora/activity-checkin.jpg",
    identity: "活跃型粉丝",
    score: "88",
  },
  {
    id: "creator-fan",
    name: "社区共创者",
    images: [
      "/img/fanora/activity-music.jpg",
      "/img/fanora/activity-community.jpg",
      "/img/fanora/activity-checkin.jpg",
      "/img/fanora/activity-concert.jpg",
    ],
    avatar: "/img/fanora/activity-music.jpg",
    identity: "贡献型粉丝",
    score: "95",
  },
  {
    id: "early-fan",
    name: "早期同行者",
    images: [
      "/img/fanora/activity-community.jpg",
      "/img/fanora/activity-badge.jpg",
      "/img/fanora/activity-concert.jpg",
      "/img/fanora/activity-checkin.jpg",
    ],
    avatar: "/img/fanora/activity-community.jpg",
    identity: "早期支持者",
    score: "90",
  },
  {
    id: "influence-fan",
    name: "粉丝传播者",
    images: [
      "/img/fanora/activity-badge.jpg",
      "/img/fanora/activity-checkin.jpg",
      "/img/fanora/activity-music.jpg",
      "/img/fanora/activity-community.jpg",
    ],
    avatar: "/img/fanora/activity-badge.jpg",
    identity: "影响力粉丝",
    score: "86",
  },
];

export const technologyPartners = [
  { id: 1, name: "Monad", detail: "Identity Layer" },
  { id: 2, name: "LangGraph", detail: "Agent Workflow" },
  { id: 3, name: "FastAPI", detail: "Backend API" },
  { id: 4, name: "Solidity", detail: "Smart Contract" },
  { id: 5, name: "viem", detail: "Web3 Client" },
];

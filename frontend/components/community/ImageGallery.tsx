...
export default function ImageGallery({ images, alt, className = "", mode = "cover" }: ImageGalleryProps) {
  ...
  if (mode === "natural") {
    if (images.length === 1) return <div className={`flex items-center justify-center overflow-hidden ${className}`}><img src={images[0]} alt={alt} className="block h-auto max-h-[80vh] w-auto max-w-full object-contain" /></div>;
    return (
      <div className={`overflow-hidden ${className}`}>
        <Swiper autoHeight modules={[Pagination]} pagination={{ clickable: true }} className="w-full [--swiper-pagination-bottom:0.75rem] [--swiper-pagination-bullet-inactive-color:#fff] [--swiper-pagination-bullet-inactive-opacity:0.45] [--swiper-pagination-color:#fff]">
          {images.map((imageUrl, index) => <SwiperSlide key={imageUrl} className="flex items-center justify-center"><img src={imageUrl} alt={`${alt} ${index + 1}`} loading={index === 0 ? "eager" : "lazy"} className="block h-auto max-h-[80vh] w-auto max-w-full object-contain" /></SwiperSlide>)}
        </Swiper>
      </div>
    );
  }

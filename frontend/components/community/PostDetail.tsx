...
export default function PostDetail({ postId, variant = "page", onClose }: PostDetailProps) {
  ...
  const content = <div className={variant === "drawer" ? "mx-auto w-full max-w-5xl px-5 pt-10 md:px-10 md:pt-12" : "container max-w-6xl"}>
    ...
    <header className={`community-reveal mx-auto max-w-4xl text-center text-white ${variant === "drawer" ? "py-5 md:py-7" : "py-10 md:py-16"}`}><span className={`inline-flex rounded-full border border-accent/25 bg-accent/10 font-bold uppercase tracking-[.2em] text-accent-lighter ${variant === "drawer" ? "px-3 py-1.5 text-[10px]" : "px-4 py-2 text-xs"}`}>{categoryLabels[post.category] || post.category}</span><h1 className={`font-display font-semibold leading-[1.12] ${variant === "drawer" ? "mt-5 text-2xl md:text-4xl" : "mt-7 text-4xl md:text-6xl"}`}>{post.title}</h1><div className={`${variant === "drawer" ? "mt-5" : "mt-7"} flex items-center justify-center gap-3`}><UserAvatar avatarUrl={post.author.avatar_url} seed={post.author.id} displayName={post.author.display_name} className={`${variant === "drawer" ? "h-9 w-9" : "h-10 w-10"} rounded-full`} /><div className="text-left"><p className={`${variant === "drawer" ? "text-xs" : "text-sm"} font-semibold`}>{post.author.display_name}</p><p className="mt-1 text-[11px] text-white/35">{post.author.level} · {formatDate(post.created_at)}</p></div></div></header>
  ...
  return variant === "drawer"
    ? <div className="community-letter-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain">{content}</div>
    : <main className="web3-page-shell min-h-screen pb-24 pt-28 md:pt-32">{content}</main>;
}

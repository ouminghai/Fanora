...
async def test_export_and_import_all_tables_except_alembic_version(tmp_path: Path):
    ...
    try:
        async with engine.begin() as connection:
            ...
            await connection.execute(text("CREATE TABLE parents (id INTEGER PRIMARY KEY, name TEXT NOT NULL)"))
            ...
            await connection.execute(text("INSERT INTO parents (id, name) VALUES (1, 'Fanora')"))
    ...
    try:
        async with engine.begin() as connection:
            ...
            await connection.execute(text("INSERT INTO parents (id, name) VALUES (2, 'stale row')"))
    finally:
        await engine.dispose()
